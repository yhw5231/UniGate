...
- **账号自动探测**（渠道可选）：开启后网关定时向上游发一道**随机 5 位数加法题**
  ...
  探测间隔**（设置页可配，默认 8 小时）再探测一次（按 (key, model) 冷却的渠道逐模型
  检查，只探测连续未使用的模型；停用/冷却中的账号不探测）；结果写入请求日志
  （`user=probe`），不计入用量统计。
...
- **空闲探测**：正常状态（渠道与 key 均启用且不在冷却）的账号**连续「空闲探测间隔」
  ...
  「设置」页调整（`probe_idle_sec`，默认 28800 秒 = 8 小时，0 = 关闭空闲探测，环境变量
...
（`user` 列为 `probe`，可在日志页过滤查看），**不计入用量统计**；活动计时为内存态，
进程重启后从首次观察到该账号起重新计满间隔。
